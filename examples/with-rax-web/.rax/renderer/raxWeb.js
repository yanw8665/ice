...
export default function runApp(appConfig, pageProps = {}) {
  ...
  if (
    pageProps &&
    Object.prototype.toString.call(pageProps) !== '[object Object]'
  ) {
  ...
  return matchInitialComponent(history.location.pathname, routes).then(
    (initialComponent) => {
      ...
      if (shell) {
        const shellData = initialDataFromSSR
          ? initialDataFromSSR[SHELL_DATA]
          : null;
        appInstance = createElement(
          shell.component,
          { data: shellData },
          appInstance
        );
      ...
      if (isWeb && rootEl === null)
        console.warn(
          'Error: Can not find #root element, please check which exists in DOM.'
        );
      // Async render.
      return render(appInstance, rootEl, { driver, hydrate });
    }
  );
}
...
function matchInitialComponent(fullpath, routes) {
  ...
  for (let i = 0, l = routes.length; i < l; i++) {
    if (
      fullpath === routes[i].path ||
      (routes[i].regexp && routes[i].regexp.test(fullpath))
    ) {
